...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["F5TTS_v1_Base", "F5TTS_v1_LoRA", "F5TTS_Base", "E2TTS_Base"],
    ...
    parser.add_argument(
        "--local_vocoder",
        action="store_true",
        help="Use local vocoder"
    )
    parser.add_argument(
        "--vocoder_path",
        type=str,
        default=None,
        help="Local path of vocoder if local vocoder is used"
    )
    parser.add_argument(
        "--use_lora",
        action="store_true",
        help="Train with LoRA",
    )
    parser.add_argument(
        "--lora_rank",
        type=int,
...
def main():
    ...
    elif args.exp_name == "F5TTS_v1_LoRA":
        wandb_resume_id = None
        model_cls = DiT
        model_cfg = dict(
            dim=1024,
            ...
            lora_feature_dim=args.lora_feature_dim,
        )
        if args.finetune:
            if args.pretrain is None:
                ckpt_path = str(cached_path("hf://SWivid/F5-TTS/F5TTS_v1_Base/model_1250000.safetensors"))
    ...
    trainer = Trainer(
        ...
        is_local_vocoder=args.local_vocoder,
        local_vocoder_path=args.vocoder_path,
        use_lora=args.use_lora,
    )
