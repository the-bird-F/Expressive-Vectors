...
def prepare_csv_wavs_dir(input_dir, num_workers=None, train_this_file=None):
    global executor
    input_dir = Path(input_dir)
    metadata_path = Path(train_this_file)
    audio_path_text_pairs = read_audio_text_pairs(metadata_path.as_posix(), input_dir)
...
def read_audio_text_pairs(csv_file_path, parent):
    audio_text_pairs = []
...
def prepare_and_save_set(inp_dir, out_dir, is_finetune: bool = True, num_workers: int = None, train_this_file: str = None):
    if is_finetune:
        assert PRETRAINED_VOCAB_PATH.exists(), f"pretrained vocab.txt not found: {PRETRAINED_VOCAB_PATH}"
    sub_result, durations, vocab_set = prepare_csv_wavs_dir(inp_dir, num_workers=num_workers, train_this_file=train_this_file)
    save_prepped_dataset(out_dir, sub_result, durations, vocab_set, is_finetune)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", type=str, required=True, help="输入 train_dataset")
    args = parser.parse_args()
    
    dataset = args.dataset
    train_this_dir = f"/inspire/hdd/global_user/chenxie-25019/pengchaofeng/Dialect_Dataset/Speech_use/{dataset}/"  # csv目录
    inp_dir = f"/inspire/hdd/global_user/chenxie-25019/pengchaofeng/Dialect_Dataset/SpeechDialects/{dataset}" # csv中语音数据前缀目录，最好记录绝对路径
    out_dir = f"/inspire/hdd/global_user/chenxie-25019/pengchaofeng/F5-TTS/data/{dataset}" #输出目录
    
    prepare_and_save_set(inp_dir, out_dir+"_pinyin", is_finetune=True, num_workers=MAX_WORKERS, train_this_file=train_this_dir+"train.csv")
    prepare_and_save_set(inp_dir, out_dir+"_dev_pinyin", is_finetune=True, num_workers=MAX_WORKERS, train_this_file=train_this_dir+"dev.csv")
    prepare_and_save_set(inp_dir, out_dir+"_eval_pinyin", is_finetune=True, num_workers=MAX_WORKERS, train_this_file=train_this_dir+"eval.csv")