...
class ConvPositionEmbedding(nn.Module):
    ...
    def forward(self, x: float["b n d"], mask: bool["b n"] | None = None, lora_idx: torch.Tensor | int | None = None):
        if mask is not None:
        ...
        for i, block in enumerate(self.conv1d):
            if self.use_lora and i in self.lora_map:
                lora_block = self.lora_conv1d[self.lora_map[i]]
                x = block(x) + lora_block(x, lora_idx)
            else:
                x = block(x)
...
class ConvNeXtV2Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, lora_idx: torch.Tensor | int | None) -> torch.Tensor:
        ...
        x = self.norm(x)
        if self.use_lora:
            x = self.pwconv1(x) + self.lora_conv1(x, lora_idx)
        else:
            x = self.pwconv1(x)
        x = self.act(x)
        x = self.grn(x)
        if self.use_lora:
            x = self.pwconv2(x) + self.lora_conv2(x, lora_idx)
        else:
            x = self.pwconv2(x)
        return residual + x
...
class AdaLayerNorm(nn.Module):
    ...
    def forward(self, x, emb=None, lora_idx: torch.Tensor | int | None = None):
        emb = self.silu(emb)
        if self.use_lora:
            emb = self.linear(emb) + self.lora_linear(emb, lora_idx)
        else:
            emb = self.linear(emb)
...
class AdaLayerNorm_Final(nn.Module):
    ...
    def forward(self, x, emb, lora_idx: torch.Tensor | int | None = None):
        emb = self.silu(emb)
        if self.use_lora:
            emb = self.linear(emb) + self.lora_linear(emb, lora_idx)
        else:
...
class FeedForward(nn.Module):
    ...
    def forward(self, x, lora_idx: torch.Tensor | int | None = None):
        if self.use_lora:
            x = self.ff[0][0](x) + self.lora_project_in(x, lora_idx)
        else:
            x = self.ff[0][0](x)
        x = self.ff[1](self.ff[0][1](x))
        if self.use_lora:
            x = self.ff[2](x) + self.lora_project_out(x, lora_idx)
        else:
            x = self.ff[2](x)
        return x
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if c is not None:
            return self.processor(self, x, c=c, mask=mask, rope=rope, c_rope=c_rope, lora_idx=lora_idx)
        else:
            return self.processor(self, x, mask=mask, rope=rope, lora_idx=lora_idx)
...
class AttnProcessor:
    ...
    def __call__(
        ...
        if attn.use_lora:
            query += attn.lora_q(x, lora_idx)
            key += attn.lora_k(x, lora_idx)
            value += attn.lora_v(x, lora_idx)
        ...
        # linear proj
        if attn.use_lora:
            x = attn.to_out[0](x) + attn.lora_to_out(x, lora_idx)
        else:
            x = attn.to_out[0](x)
        # dropout
        x = attn.to_out[1](x)
...
class JointAttnProcessor:
    ...
    def __call__(
        ...
        if attn.use_lora:
            c_query += attn.lora_q_c(c, lora_idx)
            c_key += attn.lora_k_c(c, lora_idx)
            c_value += attn.lora_v_c(c, lora_idx)
        ...
        # linear proj
        if attn.use_lora:
            x = attn.to_out[0](x) + attn.lora_to_out
        else:
            x = attn.to_out[0](x)
        # dropout
        x = attn.to_out[1](x)
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, t, mask=None, rope=None, lora_idx: torch.Tensor | int | None = None):  # x: noised input, t: time embedding
        ...
        norm, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.attn_norm(x, emb=t, lora_idx=lora_idx)

        # attention
        attn_output = self.attn(x=norm, mask=mask, rope=rope, lora_idx=lora_idx)

        # process attention output for input x
        x = x + gate_msa.unsqueeze(1) * attn_output

        norm = self.ff_norm(x) * (1 + scale_mlp[:, None]) + shift_mlp[:, None]
        ff_output = self.ff(norm, lora_idx=lora_idx)
...
class MMDiTBlock(nn.Module):
    ...
    def forward(self, x, c, t, mask=None, rope=None, c_rope=None, lora_idx: torch.Tensor | int | None = None):  # x: noised input, c: context, t: time embedding
        ...
        if self.context_pre_only:
            norm_c = self.attn_norm_c(c, t, lora_idx=lora_idx)
        else:
            norm_c, c_gate_msa, c_shift_mlp, c_scale_mlp, c_gate_mlp = self.attn_norm_c(c, emb=t, lora_idx=lora_idx)
        norm_x, x_gate_msa, x_shift_mlp, x_scale_mlp, x_gate_mlp = self.attn_norm_x(x, emb=t, lora_idx=lora_idx)

        # attention
        x_attn_output, c_attn_output = self.attn(x=norm_x, c=norm_c, mask=mask, rope=rope, c_rope=c_rope, lora_idx=lora_idx)
        ...
        if self.context_pre_only:
            ...
            c_ff_output = self.ff_c(norm_c, lora_idx=lora_idx)
        ...
        x_ff_output = self.ff_x(norm_x, lora_idx=lora_idx)
...
class TimestepEmbedding(nn.Module):
    ...
    def forward(self, timestep: float["b"], lora_idx: torch.Tensor | int | None = None):
        ...
        for i, layer in enumerate(self.time_mlp):
            if self.use_lora and i in self.lora_map:
                lora_layer = self.lora_mlp[self.lora_map[i]]
                time_hidden = layer(time_hidden) + lora_layer(time_hidden, lora_idx)
            else:
...
class LoRALinear(nn.Module):
    def __init__(
        ...
        if lora_feature_dim is None:
            ...
            self.decoders = nn.Linear(lora_rank, out_features, bias=bias)
        else:
            self.encoders = nn.ModuleList(
                nn.Linear(in_features, lora_rank, bias=False) for _ in range(lora_feature_dim)
            )
            self.decoders = nn.ModuleList(
                nn.Linear(lora_rank, out_features, bias=bias) for _ in range(lora_feature_dim)
            )
        self.init_parameters()

    def init_parameters(self):
        if self.lora_feature_dim is None:
            nn.init.normal_(self.encoders.weight, 0, 1)
            nn.init.constant_(self.decoders.weight, 0)
            if self.decoders.bias is not None:
                nn.init.constant_(self.decoders.bias, 0)
        else:
            for encoder in self.encoders:
                nn.init.normal_(encoder.weight, 0, 1)
            for decoder in self.decoders:
                nn.init.constant_(decoder.weight, 0)
    ...
    def forward(self, X: torch.Tensor, feature_idx: None | int | torch.Tensor = None) -> torch.Tensor:
        """
        Parameters:
            X: Input tensor of shape (batch_size, *seq_length, in_features).
            feature_idx: If lora_feature_dim is not None, specify which feature to use.
        """
        if isinstance(feature_idx, torch.Tensor) and feature_idx.ndim == 0:
            feature_idx = feature_idx.item()

        if self.lora_feature_dim is None:
            hidden = self.encoders(X)
            out = self.decoders(hidden)
        else:
            if feature_idx is None:
                raise ValueError("feature_idx must be provided when lora_feature_dim is not None.")
            if isinstance(feature_idx, int):
                hidden = self.encoders[feature_idx](X)
                out = self.decoders[feature_idx](hidden)
            else:
                out = []
                for i in range(self.lora_feature_dim):
                    idx_mask = (feature_idx == i).unsqueeze(-1)
                    if idx_mask.sum() == 0:
                        continue
                    X_i = X.masked_select(idx_mask).view(-1, self.in_features)
                    hidden = self.encoders[i](X_i)
                    out_i = self.decoders[i](hidden)
                    out_tensor = torch.zeros_like(X).masked_scatter(idx_mask, out_i)
...
class LoRAEmbedding(nn.Module):
    def __init__(
        ...
        if lora_feature_dim is None:
            ...
            self.decoders = nn.Linear(lora_rank, embedding_dim, bias=bias)
        else:
            self.encoders = nn.ModuleList(
                nn.Embedding(num_embeddings, lora_rank) for _ in range(lora_feature_dim)
            )
            self.decoders = nn.ModuleList(
                nn.Linear(lora_rank, embedding_dim, bias=bias) for _ in range(lora_feature_dim)
            )
        self.init_parameters()
        ...
        if self.lora_feature_dim is None:
            ...
            if isinstance(feature_idx, int):
                ...
                for i in range(self.lora_feature_dim):
                    ...
                    if idx_mask.sum() == 0:
                        continue
                    X_i = X.masked_select(idx_mask).view(-1)
                    hidden = self.encoders[i](X_i)
                    out_i = self.decoders[i](hidden)
                    out_tensor = torch.zeros(*X.shape, self.embedding_dim, device=X.device).masked_scatter(idx_mask, out_i)
                    out.append(out_tensor)
                out = torch.stack(out, dim=0).sum(dim=0)
    ...
    def __init__(
        ...
        **kwargs
        ...
        if lora_feature_dim is None:
            ...
            self.decoders = nn.Conv1d(lora_rank, out_channels, 1, bias=bias)
        else:
            self.encoders = nn.ModuleList(
                nn.Conv1d(in_channels, lora_rank, kernel_size, **kwargs) for _ in range(lora_feature_dim)
            )
            self.decoders = nn.ModuleList(
                nn.Conv1d(lora_rank, out_channels, 1, bias=bias) for _ in range(lora_feature_dim)
            )
        self.init_parameters()
    ...
    def forward(self, X: torch.Tensor, feature_idx: None | int | torch.Tensor = None) -> torch.Tensor:
        ...
        if self.lora_feature_dim is None:
            ...
            if isinstance(feature_idx, int):
                ...
                out = []
                for i in range(self.lora_feature_dim):
                    idx_mask = (feature_idx == i).unsqueeze(-1).unsqueeze(-1)
                    if idx_mask.sum() == 0:
                        continue
                    X_i = X.masked_select(idx_mask).view(-1, self.in_channels, X.shape[-1])
                    hidden = self.encoders[i](X_i)
                    out_i = self.decoders[i](hidden)
                    out_tensor = torch.zeros_like(X).masked_scatter(idx_mask, out_i)
                    out.append(out_tensor)
